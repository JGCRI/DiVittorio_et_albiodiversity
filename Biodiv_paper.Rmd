...
# must run the first two chunks in order to run any of the subsequent chunks
...
GCAM_defaultnoprot <- rgcam::loadProject("outputs/project_files/tables_gcam_defaultnoprot.proj")
GCAM_defaultNZnoprot <- rgcam::loadProject("outputs/project_files/tables_gcam_defaultNZnoprot.proj")
Allansce1noprot <- rgcam::loadProject("outputs/project_files/tables_allansce1noprot.proj")
Allansce1NZnoprot <- rgcam::loadProject("outputs/project_files/tables_allansce1nznoprot.proj")
Allansce2noprot <- rgcam::loadProject("outputs/project_files/tables_allansce2noprot.proj")
Allansce2NZnoprot <- rgcam::loadProject("outputs/project_files/tables_allansce2nznoprot.proj")
perc30noprot <- rgcam::loadProject("outputs/project_files/tables_30percnoprot.proj")
perc30NZnoprot <- rgcam::loadProject("outputs/project_files/tables_30percnznoprot.proj")
...
ggsave( paste0("outputs/images/",'1. Crop_prices','.pdf'),width = 10.5, height = 10)
...
# box plots of marginal protection cost/land conversion pressure
...
#          region=="Africa_Western"), aes(x=year,y=norm_ratio,color="Forests in the Congo basin"),linetype="dashed",linewidth=1.3,show.legend = T)+
     ...
     facet_wrap(~scenario,scales="free", nrow=2)+
     ylim(0.75,1.5)+
     ylab("Relative protected land conversion pressure (wrt to CURRENT)")
...
write.csv(increase_in_costs_NZ, "outputs/csv/Fig4B_Protected_area_land_rent.csv")
...
ggsave( paste0("outputs/images/",'4B. Protected_area_land_rent','.png'),width = 15, height = 10)
...
# plot mitigation scenario values
 ...
 write.csv(tot_prot_cost_more, "outputs/csv/protection_cost_region.csv")
...
Protected_Land <- getQuery(GCAM_default, "detailed land allocation") %>%
  mutate(scenario="CURRENT ref") %>% bind_rows(getQuery(Allansce1, "detailed land allocation") %>%mutate(scenario="BIODIV ref")) %>%
  bind_rows(getQuery(Allansce2, "detailed land allocation") %>%mutate(scenario="BIODIV30 ref")) %>%
  bind_rows(getQuery(perc30, "detailed land allocation") %>%mutate(scenario="UNIFORM30 ref")) %>%
   bind_rows(getQuery(GCAM_defaultNZ, "detailed land allocation") %>%
  mutate(scenario="CURRENT LCT")) %>% bind_rows(getQuery(Allansce1NZ, "detailed land allocation") %>%mutate(scenario="BIODIV LCT")) %>%
  bind_rows(getQuery(Allansce2NZ, "detailed land allocation") %>%mutate(scenario="BIODIV30 LCT")) %>%
  bind_rows(getQuery(perc30NZ, "detailed land allocation") %>%mutate(scenario="UNIFORM30 LCT"))   %>%  
...
Protected_Land_no_prot <- getQuery(GCAM_defaultnoprot, "detailed land allocation") %>%
  mutate(scenario="CURRENT ref") %>% bind_rows(getQuery(Allansce1noprot, "detailed land allocation") %>%mutate(scenario="BIODIV ref")) %>%
  bind_rows(getQuery(Allansce2noprot, "detailed land allocation") %>%mutate(scenario="BIODIV30 ref")) %>%
  bind_rows(getQuery(perc30noprot, "detailed land allocation") %>%mutate(scenario="UNIFORM30 ref")) %>%
   bind_rows(getQuery(GCAM_defaultNZnoprot, "detailed land allocation") %>%
  mutate(scenario="CURRENT LCT")) %>% bind_rows(getQuery(Allansce1NZnoprot, "detailed land allocation") %>%mutate(scenario="BIODIV LCT")) %>%
  bind_rows(getQuery(Allansce2NZnoprot, "detailed land allocation") %>%mutate(scenario="BIODIV30 LCT")) %>%
  bind_rows(getQuery(perc30NZnoprot, "detailed land allocation") %>%mutate(scenario="UNIFORM30 LCT"))   %>%  
  filter(grepl("Protected",landleaf)) %>% 
...
g <- ggplot(data=diff_data %>% filter(year==2100), aes(x=scenario,y=(diff/tot_land)*100,color=type))+
     geom_boxplot(outlier.shape = NA)+
     coord_cartesian(ylim = c(0, 4.5))+  
     ylab("Loss of protected area by not imposing protection (% of protected land)")
  
g+scheme_basic+theme(axis.title.y=element_text(size=14))+scale_color_manual(values=c("#005AB5", "#FFC20A", "#DC3220"))

ggsave("outputs/images/protected_area_lost_boxplot.pdf", width = 15, height = 10)


write.csv(diff_data, "outputs/csv/protected_area_lost.csv")

```


#2. Generate scatterplot with r squared and p values between area loss and profit rates
# this requires the previous chunkk for diff_data
```{r}
Profit_rate <- getQuery(GCAM_default, "profit rate") %>%
               mutate(scenario="a) CURRENT reference") %>% bind_rows(getQuery(Allansce1, "profit rate") %>%mutate(scenario="b) BIODIV reference")) %>%
  bind_rows(getQuery(Allansce2, "profit rate") %>%mutate(scenario="c) BIODIV30 reference")) %>%
...
diff_data %>% mutate(scenario= ifelse( scenario == "CURRENT ref", "a) CURRENT reference", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "BIODIV ref", "b) BIODIV reference", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "BIODIV30 ref", "c) BIODIV30 reference", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "UNIFORM30 ref", "d) UNIFORM30 reference", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "CURRENT LCT", "e) CURRENT LCT", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "BIODIV LCT", "f) BIODIV LCT", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "BIODIV30 LCT", "g) BIODIV30 LCT", scenario ) ) %>%
    mutate(scenario= ifelse( scenario == "UNIFORM30 LCT", "h) UNIFORM30 LCT", scenario ) )-> diff_data_relabelled  
 
Profit_rates <- increase_in_costs %>%
    select(-value,-Units) %>% 
    left_join(diff_data_relabelled)

write.csv(Profit_rates, "outputs/csv/protected_loss_vs_marginal_cost.csv")

g <- ggplot(data=Profit_rates %>% filter(year==2100) %>% na.omit(),aes(x=ratio,y=(diff/tot_land)*100,color=type)) + 
     geom_point()+
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
    #stat_cor(aes(label = after_stat(rr.label), label.x = 3, label.y = 30,color="blue") + 
    stat_poly_eq(aes(label = paste(after_stat(eq.label), after_stat(rr.label), after_stat(p.value.label), sep = "~~~")),
     ...
     xlim(0,NA)+
     ylab("Loss of protected area by not imposing protection (% of protected land)")+
     xlab("Relative protected land conversion pressure")+
     scale_color_npg()

g+scheme_basic+theme(axis.title.y=element_text(size=14))+scale_color_manual(values=c("#005AB5", "#FFC20A", "#DC3220"))

ggsave("outputs/images/protected_loss_vs_marginal_cost.pdf", width = 15, height = 10)
