...
Profit_rate <- getQuery(GCAM_default, "profit rate") %>%
  ...
  bind_rows(getQuery(GCAM_defaultNZ, "profit rate") %>%mutate(scenario="e) CURRENT mitigation")) %>%
  bind_rows(getQuery(Allansce1NZ, "profit rate") %>%mutate(scenario="f) BIODIV mitigation")) %>%
  bind_rows(getQuery(Allansce2NZ, "profit rate") %>%mutate(scenario="g) BIODIV30 mitigation")) %>%
  bind_rows(getQuery(perc30NZ, "profit rate") %>%mutate(scenario="h) UNIFORM30 mitigation")) %>%
...
mit_temp = filter(increase_in_costs,
                  scenario == "e) CURRENT mitigation" | scenario == "f) BIODIV mitigation" | scenario == "g) BIODIV30 mitigation" | scenario == "h) UNIFORM30 mitigation")
mit_temp = merge(mit_temp, filter(mit_temp, scenario == "e) CURRENT mitigation") %>% mutate(ratio_current=ratio, scenario=NULL, value=NULL, unmanaged=NULL, ratio=NULL),
                 by=c("Units", "region", "landleaf", "year"))
...
g <- ggplot(data=increase_in_costs %>% filter(year>=2015), aes(x=year,y=ratio),show.legend=F)+
     ...
     ylim(0,3)+
     ylab("Marginal cost of protection relative to unmanaged land value")
...
panel_names = c("a) CURRENT reference", "b) BIODIV reference", "c) BIODIV30 reference", "d) UNIFORM30 reference",
                "e) CURRENT mitigation", "f) BIODIV mitigation", "g) BIODIV30 mitigation", "h) UNIFORM30 mitigation")
...
g <- ggplot(data=increase_in_costs %>% filter(year>2015 & scenario != "a) CURRENT reference" & scenario != "e) CURRENT mitigation"), aes(x=year,y=norm_ratio),show.legend=F)+
     geom_boxplot(aes(group=paste0(year,scenario)),outlier.shape = NA)+
  
#     geom_line(data=increase_in_costs %>% 
#          filter(year>2015 & scenario != "a) CURRENT reference" & scenario != "e) CURRENT mitigation",grepl("ProtectedUnmanagedForest_AmazonR",landleaf),
...
#     geom_line(data=increase_in_costs %>% 
#          filter(year>2015 & scenario != "a) CURRENT reference" & scenario != "e) CURRENT mitigation",grepl("ProtectedShrubland_AusInt",landleaf),
...
#     geom_line(data=increase_in_costs %>% 
#          filter(year>2015 & scenario != "a) CURRENT reference" & scenario != "e) CURRENT mitigation",grepl("ProtectedUnmanagedForest_CongoR",landleaf),
#          region=="Africa_Western"), aes(x=year,y=norm_ratio,color="Forests in the Congo basin"),linetype="dashed",linewidth=1.3,show.legend = T)+
     ...
     facet_wrap(~scenario,scales="free", nrow=2)+
     ylim(0.75,1.5)+
     ylab("Relative marginal cost of protection normalized to CURRENT")
